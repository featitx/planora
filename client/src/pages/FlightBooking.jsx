...
const FlightBooking = ({ onBack }) => {
  ...
  const [currentFlight, setCurrentFlight] = useState(location.state?.flight || null);
  ...
  if (!currentFlight) {
    return <div className="text-center py-12">Flight details not found or loading...</div>;
  }
  ...
  const handleConfirmBooking = async () => {
    if (!currentFlight || !currentFlight._id) {
      toast.error("Flight details missing. Please go back and select a flight.");
      return;
    }
          ...
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="font-semibold text-gray-800">{currentFlight.airline}</h3>
                  <p className="text-sm text-gray-600">{currentFlight.flightNumber}</p>
                </div>
              </div>
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                currentFlight.class === 'Economy' ? 'bg-blue-100 text-blue-800' :
                currentFlight.class === 'Premium Economy' ? 'bg-purple-100 text-purple-800' :
                currentFlight.class === 'Business' ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="flex items-center justify-between">
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-800">{currentFlight.departure.time}</p>
                <p className="text-sm text-gray-600">{currentFlight.departure.code}</p>
                <p className="text-xs text-gray-500">{currentFlight.departure.city}</p>
              </div>
              
              <div className="flex-1 px-4">
                <div className="text-center mb-2">
                  <p className="text-sm text-gray-600">{formatDuration(currentFlight.duration)}</p>
                </div>
                  ...
                  <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-1">
                    {currentFlight.stops === 0 ? (
                      ...
                      <div className="text-xs text-gray-600 bg-white px-1">
                        {currentFlight.stops} stop{currentFlight.stops > 1 ? 's' : ''}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-800">{currentFlight.arrival.time}</p>
                <p className="text-sm text-gray-600">{currentFlight.arrival.code}</p>
                <p className="text-xs text-gray-500">{currentFlight.arrival.city}</p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sticky top-8">
          ...
          <div className="space-y-4 mb-6">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">₹{currentFlight.price * passengers.length}</span>
            </div>
            <div className="flex justify-between">
